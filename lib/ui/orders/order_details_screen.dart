...
class OrderDetailsScreen extends StatelessWidget {
  ...
  const OrderDetailsScreen({Key? key, required this.orderModel, this.showAppBar = true}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return GetBuilder<OrderDetailsController>(
        ...
        builder: (controller) {
          // Set the controller's orderModel if not already set
          if (controller.orderModel.value.id == null || controller.orderModel.value.id != orderModel.id) {
            controller.orderModel.value = orderModel;
          }
          return Scaffold(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      ButtonThem.buildButton(
                                        ...
                                        btnHeight: 44,
                                        onPress: () async {
                                          // Handle payment cancellation/refund logic
                                          if (orderModel.paymentType?.toLowerCase() == "stripe" && 
                                              orderModel.paymentIntentId != null) {
                                            // Cancel Stripe pre-authorization
                                            try {
                                              final response = await http.post(
                                                Uri.parse('https://api.stripe.com/v1/payment_intents/${orderModel.paymentIntentId}/cancel'),
                                                headers: {
                                                  'Authorization': 'Bearer ${controller.orderModel.value.service?.adminCommission?.amount}', // You'll need to get the stripe secret
                                                  'Content-Type': 'application/x-www-form-urlencoded',
                                                },
                                              );
                                              if (response.statusCode == 200) {
                                                print("Pre-authorization cancelled successfully");
                                              }
                                            } catch (e) {
                                              print("Error cancelling pre-authorization: $e");
                                            }
                                          } else if (orderModel.paymentType?.toLowerCase() == "wallet") {
                                            // Refund wallet amount
                                            final amount = orderModel.finalRate ?? orderModel.offerRate ?? "0";
